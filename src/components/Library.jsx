...
const Library = () => {
  ...
  const errorMessage = searchValue ? 'Error nothing was found.' : `You don't have any saved movies.`
  ...
  return (
    <div className='w-screen bg-[#1E1E1E] pt-[40px] pl-[26px]'>
      ...
      {
        filteredMovies.length === 0 ? <Error value={errorMessage}/> :
          <div className='flex gap-[15px] flex-wrap'>
            {searchValue ? filteredMovies.map((movie) => {
              return <Movie recentHandler={() => recentHandler(movie)} key={movie['#IMDB_ID']} movie={movie}/>
